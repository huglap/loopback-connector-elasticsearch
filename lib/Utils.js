const {of, from,iif,isObservable,EMPTY, throwError } = require("rxjs")
...
class Elasticsearch_Connector_Utils {
    ...
    mapResult$(qr){
        if(qr instanceof Error) return throwError(qr)
        if (qr.hasOwnProperty("statusCode") && !String(qr.statusCode).match(/^2/)){
            console.log(qr.statusCode)
            let err = new Error()
            err.status = qr.statusCode
            err.name = "Elasticsearch error"
            err.message = JSON.stringify(qr.body)
