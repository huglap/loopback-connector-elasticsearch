...
class Elasticsearch_Connector extends Connector {
    ...
    async findById(id, model){
        ...
        if(body && body.hasOwnProperty("_source")) return [body._source]
        else return []
    ...
    async findAll(model,filter) {
        ...
        const query = await Utils.$buildQuery(model, filter)
        const result = await this.executeQuery(query,model,"findAll")
        return result
    }

    async create(model, data, done){
        let idValue = this.getIdValue(model, data);
        let idName  = this.idName(model);
        let query = Object.assign({},data)

        try {
            const result = await this.executeQuery(query,model,"create")
            return []
        } catch(e){
            log(e)
            return []
